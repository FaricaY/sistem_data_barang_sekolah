...
<div class="flex items-center justify-center p-8">
    <div class="w-full max-w-2xl">
        <div class="bg-white p-8 rounded-2xl shadow-xl border border-gray-200">
            <h1 class="text-3xl font-bold text-center text-gray-800 mb-8">Edit Item</h1>
            ...
            <form action="{{ route('items.update', $item->id) }}" method="POST">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="code" id="item_code" value="{{ $item->code }}"
                               class="w-full px-4 py-2 border border-purple-300 rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-500 transition">
                    </div>

                    <!-- Item Name -->
                    <div>
                        <label for="item_name" class="block text-sm font-semibold text-gray-700 mb-1">Item Name</label>
                        <input type="text" name="name" id="item_name" value="{{ $item->name }}"
                    ...
                    <div>
                        ...
                        <select name="condition_id" id="condition" class="w-full px-4 py-2 border border-purple-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition">
                            @foreach($conditions as $condition)
                                <option value="{{ $condition->id }}" {{ $item->condition_id == $condition->id ? 'selected' : '' }}>
